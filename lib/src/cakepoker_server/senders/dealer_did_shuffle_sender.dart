...
class DealerDidShuffleSender {
  final Reader read;
  DealerDidShuffleSender(this.read);

  sendDealerDidShuffleMessgae(CakepokerRecord record) async {
    final state = read(gameRoomState);
    final userIds = state.players.map((player) => player.userId).toList();
    final message = GameMessage(
      type: GameMessageType.dealerDidShuffle,
